...
<!--
BUDGET NOTE. THE `budget:` FIELD IS GONE -- docs/adr/0010 deleted it from all three
files that declared it, on the measurement that a line meant 71 bytes in one of them
and 352 in another, so the field ranked the three in the opposite order from their
size and called the largest compliant. Nothing reads it now and C7, the check that
did, is retired.

WHAT SURVIVES IS THE RULE, WHICH WAS NEVER ABOUT THE NUMBER: past the point where
this file is too long to read, move narrative out rather than raise a threshold. The
history below is why that rule is worth keeping and is left exactly as written.

Task 53 specified 250, assuming each row could link to the document owning the
...
(`backend/api/app.py:695`, `:699`) that `api/tests/test_worker_check.py` pins with a fake connection.
...
(`backend/api/contributor-worker/google-serpapi-worker.py:619`), so an operator who sets
...
- **(a) `--install` asks the server once.** Costs no SerpApi credit — only a claimed *search* does —
  but `claim` is the only route returning `poll_interval_seconds` (`backend/api/app.py:530`), and
...
reported it. `api/app.py:764-771` already carries a mitigation — two constants parked at the bottom
...
form entirely** (`backend/tools/audit-citations.py:18-26`, the `T-18` blindspot), so it trades a
...
(`backend/api/app.py:914`). That bounds what the process **holds**: an oversized `Content-Length` is
