...
> nightly pipeline is at the root (`run-daily.py` and what it calls); the
> contributor API is under `api/`. They were separate repos until slice D of
...
```bash
python3 migrate_scores.py                       # dry run — reports, changes nothing
python3 migrate_scores.py --apply               # copy scores into job_scores
python3 migrate_scores.py --apply --drop-columns  # ...and remove the old columns
```
...
```bash
python3 backfill-scores.py --dry-run              # plan only
