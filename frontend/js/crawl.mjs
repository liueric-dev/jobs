...
// filter among the eight query parameters (backend/webapp/jobs.py:412-420).
...
// minted server-side per render (jobs.py:181-196, :464) and POST /v1/events
// refuses a batch without one (`missing_request_id`, jobs.py:682-685). Both are
...
// (jobs.py:210-232) -- so ranks run 1..N across the whole crawl and every
...
// (jobs.py:772-781), which turns every impression above an `open` into a
 ...
 * list hides them by default (jobs.py:458-459) and `include_dismissed` is
 * documented as debugging only and explicitly not part of the client contract
 * (jobs.py:419-420, :456-457), so this does not reach for it.
 */
