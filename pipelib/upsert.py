...
WHAT STAYS OUT
    No table DDL lives here. `public.events` and `jobs.jobs` are owned by
    their pipelines; this module only needs a TableSpec describing which
    columns to write.
"""
...
class TableSpec:
    ...
    def select_sql(self):
        cols = "content_hash" + (f", {self.revive_column}"
                                 if self.revive_column else "")
...
def upsert(conn, spec, records, id_fn, *, now=None, debug=False):
    ...
    different fields, so it stays a caller concern (see pipelib.ids.make_id).
