...
USAGE
    python3 jobs/migrate_profiles.py                     # report, change nothing
    python3 jobs/migrate_profiles.py --apply             # create/refresh
    python3 jobs/migrate_profiles.py --apply --bump      # ... and invalidate matches
...
def main():
    ...
    if args.bump and matched:
        print(f"  {matched} job_matches rows are now stale -- "
              f"run jobs/match.py to recompute.")
    conn.close()
