...
- Each additional machine just needs its own `SERPAPI_API_KEY` in its own `~/.hermes/.env` and a synced copy of `config/google-queries.json`.
- **Script distribution (decided 2026-07-24, not yet done): git, pull-before-run.** The plan for running from the laptops: make `~/.hermes/scripts/jobs/` a git repo (bare repo hosted on the server over Tailscale/SSH, or a private GitHub repo — either works), and each worker machine runs `git pull --ff-only && python3 run-daily.py` as its entry point. This beats ad-hoc copying/`scp`-ing from the server because every machine states exactly which version it ran, updates are atomic, and a broken edit on one machine can't silently propagate. Phones: technically runnable (Termux on Android can do Python+psycopg over Tailscale) but not worth the upkeep — the min-interval guard means extra machines add resilience and quota, not coverage, so two laptops + server is already plenty.
...
**Multi-user crowdsourcing — BUILT, lives outside this directory (2026-07-24):**

Implemented as a standalone service at **`~/apps/jobs-api/`** (its own git repo). It is deliberately *not* here: `~/.hermes` is the Hermes harness's private directory for scripts and cron jobs, and this is a persistent, eventually internet-facing server. See that repo's `README.md` for API details, security model, and deployment steps.

- **Shape:** contributors get an admin-issued API key (`manage_users.py`), pull claimed queries from `POST /v1/queries/claim`, run them against SerpApi with **their own** key, and `POST .../submit` the raw results. Only this service talks to Postgres — no contributor ever gets DB credentials. Apify is deliberately *not* exposed (it bills Eric's account per result).
- **No code is shared between the two systems, on purpose.** `~/apps/jobs-api/query_claims.py` reimplements the claim algorithm rather than importing from here. They coordinate through the *same Postgres row* (`job_ingest_state`, `google_jobs:query:<slug>`) using the same atomic conditional update, so row-level locking extends the "two claimants never get the same query" guarantee across both codebases automatically. Verified live: 18 concurrent claims across two contributors, zero overlap.
...
All scripts resolve sibling files relative to their own location (`ingest/` scripts go one level up to reach `config/`), not the process's working directory — so the whole folder can still be moved as a unit. Run ingest scripts from the `jobs/` root as `python3 ingest/<name>.py` (or from anywhere — CWD doesn't matter).

**Subdirectory scripts work fine with `hermes cron`** — despite an earlier assumption in this codebase's own comments that "cron scripts must live [directly in ~/.hermes/scripts/]", that turned out not to be a real constraint. Confirmed live: `hermes cron edit 43f2e0330e75 --script jobs/run-daily.py` was accepted and the job ran correctly from the subdirectory.
...
Scheduled: `hermes cron job daily-jobs-ingest`, id `43f2e0330e75`, daily at midnight.
