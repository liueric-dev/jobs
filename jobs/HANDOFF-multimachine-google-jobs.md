...
Run `jobs/ingest/google-serpapi.py` from several machines, each with its own SerpApi key,
...
Repeatable via `jobs/tools/verify-date-filter.py` (`--dry-run` spends nothing).
...
- **New:** `jobs/tools/verify-date-filter.py`
...
- **`jobs/ingest/google-serpapi.py`**, **`jobs/ingest/google-apify.py`** — `normalize_job()`
...
- **New:** `jobs/migrate_google_ids.py` — dry-run default, `--apply`, idempotent.
...
**Before `--apply`:**
1. Confirm nothing is running: `pgrep -af "jobs/score.py"` returns nothing.
2. Find out *what* is relaunching it. `hermes cron` has `daily-jobs-ingest` at `0 0 * * *`
   (`jobs/run-daily.py`, enabled) — that does not explain a 05:55 start. Suspect a manual run,
   another agent session, or a loop.
3. Re-take the backup if significant time has passed.
4. Then: `python3 jobs/migrate_google_ids.py` (confirm ~837→632), then `--apply`.
...
- DDL in `jobs/schema.py:ensure_schema()`, next to `google_jobs_query_stats`.
...
30 2 * * * cd ~/hermes-scripts && git pull --ff-only && \
  /usr/bin/python3 jobs/ingest/google-serpapi.py >> ~/hermes-jobs.log 2>&1
```

### Step 7 — docs
- `jobs/README.md` — fix the wrong "shared not multiplied" claim (see above).
...
| File | State |
|---|---|
| `jobs/tools/verify-date-filter.py` | **new**, run, verified |
| `pipelib/ids.py` | modified — identity helpers |
| `pipelib/tests/test_pipelib.py` | modified — `TestGoogleJobIdentity` (92/92 pass) |
| `jobs/ingest/google-serpapi.py` | modified — `normalize_job()` |
| `jobs/ingest/google-apify.py` | modified — `normalize_job()`, dropped unused `hashlib` |
| `jobs/migrate_google_ids.py` | **new**, dry-run verified, **not applied** |
...
`events/nyc-events-ingest.py`, `events/nyc-library-events-ingest.py`, `jobs/README.md`,
`jobs/ingest/builtin-nyc.py`, `jobs/score.py`, `jobs/tools/compare-models.py`,
`pipelib/geocode.py`, `pipelib/llm.py`, `jobs/backfill-scores.py`, `jobs/tools/cost-test.py`,
