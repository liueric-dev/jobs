...
python3 jobs/run-daily.py

# individually
python3 jobs/extract.py                       # facts for new postings
python3 jobs/match.py                         # re-rank stale rows only
python3 jobs/match.py --rebuild --dry-run     # what a full re-rank would do
python3 jobs/score.py --profile tech --limit 20

# burn down an extraction backlog
EXTRACT_MAX_WORKERS=16 ./jobs/backfill-facts.sh

# add or update a profile
$EDITOR jobs/config/criteria.json
python3 jobs/migrate_profiles.py --apply --bump   # --bump invalidates matches
python3 jobs/match.py

# is the ranking good enough?
python3 jobs/tools/calibrate-match.py --disagreements 15
```
