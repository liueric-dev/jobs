...
# HANDOFF: extraction fields and match quality — ARCHIVED

> **Archived 2026-08-01 from `backend/docs/HANDOFF-match-quality.md` (task 40), where it
> was dispositioned by task 34 § D and had not moved.** Frozen at its write date and not
> maintained. A stub and a link remain at the original path.
>
> **What it measured:** how well `match.py` ranked postings for **profile `tech`** — the
> repo author's own software-engineer job search — over 917 postings the previous
> LLM-per-pair pipeline had already scored, measured 2026-07-26 with
> `tools/calibrate-match.py` and `tools/learned-ranker-probe.py`.
>
> **What superseded it, in three pieces:**
>
> - **§ 4, the seven measurement traps** — promoted 2026-07-31 to
>   [`docs/MEASUREMENT-TRAPS.md`](../MEASUREMENT-TRAPS.md), which is the copy to read and
>   to cite. It carries later additions from the refactor run that are not below. § 4 is
>   kept here in full because §§ 3 and 5 argue against it and a reader of this document
>   needs it in place.
> - **The persona** — the pipeline is retargeted at the Pursuit AI-Native cohort: entry
>   level, AI-adjacent, all industries, NYC. Profile `tech` is not that population. See
>   [`README.md`](README.md) for why nothing measured against it transfers.
> - **The next move it recommends** — the learned ranker — is still open, and is where
>   `docs/tasks/refactor/README.md` tracks it. The recommendation survives; the numbers
>   attached to it do not.
>
> **THE 12.7/20 IN HERE IS NOT A QUALITY SCORE. Read it as imitation fidelity against a
> non-target persona.** `learned-ranker-probe.py` was fitted and cross-validated against
> `job_scores.fit_score` — the judgement of the LLM-per-pair pipeline this design
> *replaced*, for a software engineer who is not a Builder. So 12.7/20 says how closely a
> learned model reproduces **that** LLM's ranking of **that** person's corpus, and a
> perfect 20/20 would mean perfect imitation of an incumbent nobody claims was right. This
> document already says so in its own words — *"the LLM is not right, it is the
> incumbent"* — and `docs/MEASUREMENT-TRAPS.md` § *Never evaluate on the layer you trained
> on* is the general form: `fit_score` is L1, and fitting to L1 and scoring on L1 measures
> agreement, not quality. **Quoting 12.7/20 as "the ranker's precision@20" is the specific
> misreading this header exists to prevent.**

---

## HANDOFF: extraction fields and match quality — the original document
...
appears to move Spearman a lot, you have hit trap §4.1, not a real effect.
  ...
  model on the identical features and gets 12.7/20 (imitation fidelity, not quality —
  see the archive header) against the rules' 8.0, so
...
**Relabelled 2026-08-01, task 40: read every `precision@20` in this table as
*imitation fidelity against profile `tech`'s LLM labels*, not as quality.** The
labels are `job_scores.fit_score` — the incumbent this design replaced, for a
non-target persona. See the archive header.
...
4. `python3 migrations/migrate_profiles.py --apply --bump --profile tech`
...
[+0.099, +0.240] — **imitation fidelity against a non-target persona, not a quality
score; see the archive header.** So the features were never the ceiling, the weighting
