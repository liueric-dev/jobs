...
# Standing guidance — the `docs/tasks/refactor/` run

**What in this run is true until the code changes, rather than true on a date.** The
prohibitions, the operational reference for the labelling surface, the FAQ, what is blocked,
the documented claims that are wrong about the code as it stands, the GATE 2 question, and
the follow-ups no task owns.

**Split out of `HANDOFF.md` on 2026-08-02 by
[task 47](tranche_eight/47-split-the-entry-point.md), and it is the finding that task did not
predict.** 47 said the entry point was four documents. Doing the split found a **fifth**
lifecycle, and it is the largest of them: content that is neither the entry point nor a dated
record — it is a `contract`, correct until someone changes the thing it describes. Leaving it
in a `rolling` file is what made that file unreadable, and calling it a `record` would have
frozen guidance that must not freeze.

**Read it for what still binds. It is not a session's assignment**, and where a section says
*"the next session"* it means whichever session the sentence still applies to. The text is
unchanged from `HANDOFF.md`; `git log -p -- docs/tasks/refactor/HANDOFF.md` reaches it at
that path, and `§` cross-references mean sections that are now in this file, in
[`sessions/`](sessions/), or in [`../../archive/`](../../archive/).
