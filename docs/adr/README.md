...
| [0012](0012-contributor-scrape-runs-as-apify-actor.md) | The contributor scrape runs as an Apify actor in each classmate's own account, keyed by a Google sign-in restricted to one Workspace domain; the SerpApi and operator-billed Apify steps are disabled pending review — superseding `0006` 3 and `0007` 2, 4 |
