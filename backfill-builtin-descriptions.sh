...
#
#   ./backfill-builtin-descriptions.sh
#   BUILTIN_DETAIL_DELAY=10 BUILTIN_BACKOFF=1800 ./backfill-builtin-descriptions.sh
set -uo pipefail

# Was `$(dirname "$0")/..` when this script lived in jobs/ and the python it
# calls was jobs/ingest/builtin-nyc.py. Slice D flattened both to the repo root.
cd "$(dirname "$0")"
set -a; . ./.env; set +a
