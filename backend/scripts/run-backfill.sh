...
#
# The `/..` is because this script lives in scripts/ while .env, config/ and the
# python it drives are at the repo root -- everything below runs from there.
set -euo pipefail

cd "$(dirname "$(readlink -f "$0")")/.."
set -a; . ./.env; set +a

python3 scripts/backfill-scores.py --workers "${BACKFILL_WORKERS:-3}" 2>&1 \
    | tee backfill.log
