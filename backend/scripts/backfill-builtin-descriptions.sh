...
#
#   ./scripts/backfill-builtin-descriptions.sh
#   BUILTIN_DETAIL_DELAY=10 BUILTIN_BACKOFF=1800 ./scripts/backfill-builtin-descriptions.sh
set -uo pipefail

# This script lives in scripts/; .env and the ingest/ tree it calls are at the
# repo root, so everything below runs from there.
cd "$(dirname "$0")/.."
set -a; . ./.env; set +a
...
while [ "${left:-0}" -gt 0 ]; do
    round=$((round + 1))
    out=$(python3 ingest/builtin-nyc.py 2>&1)
