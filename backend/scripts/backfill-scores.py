...
USAGE
    ...
    python3 scripts/backfill-scores.py --dry-run
    ...
    python3 scripts/backfill-scores.py --rescore-stale
...
# scripts/ sits one level below the pipeline modules this imports and the
# score.py it drives. Python puts THIS file's directory on sys.path, not its
# parent, so the parent is added by hand -- and that same path is the cwd each
# score.py subprocess must run in, since score.py resolves config/ and .env
...
def run_round(batch_size, workers, timeout):
    ...
    try:
        r = subprocess.run(
            [sys.executable, os.path.join(REPO_ROOT, "score.py")],
