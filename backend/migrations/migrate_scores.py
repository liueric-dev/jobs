...
USAGE
    python3 migrations/migrate_scores.py                    # dry run: report, change nothing
    python3 migrations/migrate_scores.py --apply            # copy rows into job_scores
    python3 migrations/migrate_scores.py --apply --drop-columns   # ...and drop the old columns
...
def default_profile():
    ...
    """
    # Against the repo root, not this file's directory: config/ stayed at the
    # root when the migrations moved down into migrations/.
    persona_file = os.environ.get(
        "JOB_SCORING_PERSONA_FILE",
        os.path.join(_REPO_ROOT, "config/persona.json"),
    )
    try:
