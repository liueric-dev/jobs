...
#:
#: The tuple is asserted against the directory listing below so new SQL routes
#: cannot silently evade the privilege scan.
...
class TestEveryRouteModuleIsScanned(unittest.TestCase):
    ...
    that starts cleanly and 500s on that route with a permission error. The
    tuple is checked against the directory rather than against memory.
