...
4. Then: `python3 migrations/migrate_google_ids.py` (confirm ~837→632), then `--apply`.
...
1. Rotate `POSTGRES_PASSWORD` **and** the password inside `DATABASE_URL` in `~/apps/jobs/backend/.env`
   (they must match or compose fails fast). Also update `~/apps/jobs/backend/api/.env`, which holds
...
2. `docker-compose.yml`:
   ...
   ports:
     ...
     - "127.0.0.1:5432:5432"
   ```
3. `docker compose --env-file ~/apps/jobs/backend/.env up -d --force-recreate postgres`, then confirm
