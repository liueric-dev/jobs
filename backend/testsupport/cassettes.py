"""Record and replay HTTP responses for offline ingestion tests.

Cassette keys and stored payloads scrub secret query parameters, headers, and
known environment values. Recording dates remain visible so stale upstream
fixtures are not mistaken for current responses.
"""
...
#: Where committed cassettes live. Unlike testsupport/.cache/ these ARE tracked:
...
#: Include common API credentials even when current public feeds use none.
    ...
    def load(cls, name, directory=None):
        ...
        try:
            ...
            raise CassetteError(
                ...
                f"`python3 testsupport/record_cassettes.py {name}` -- see that "
...
def recording(name, *, source="", note="", recorded_by="", directory=None,
    ...
    testsupport/record_cassettes.py so that what was recorded and what it cost is
