"""Create a disposable schema using the real ingestion DDL.

Database-backed tests need PostgreSQL transaction behavior, including
per-record savepoints. Each test schema is isolated and removed afterwards;
the harness never inserts investigation rows into the existing jobs table.
"""
...
def available():
    ...
    and silence there would mask a genuine driver bug as the identical skip to every module
...
def scratch_schema(url=None, keep=False):
    ...
    import schema as jobs_schema        # imported here: sys.path is a caller
                                        ...
                                        # by testsupport/ code that has already set it
