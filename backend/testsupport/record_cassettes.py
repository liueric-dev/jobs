#!/usr/bin/env python3
"""Record ingestion cassettes against live public upstreams.

Run with --list for available recipes or --all-free for the public sources.
Each recipe uses the corresponding production fetcher and writes scrubbed
responses to testsupport/fixtures/cassettes. Recording changes fixtures;
ordinary tests only replay them offline.
"""
...
RECORDED_BY = "testsupport/record_cassettes.py"
...
def record_ats_greenhouse_no_content():
    ...
    No live board offers a missing `content` field when it is requested:
    every posting on every board carries the field when it is asked
...
def record_ats_greenhouse_domsoup():
    ...
    string for the reason  states: fixtures written from a
    ...
    with cassettes.recording(
            ...
            note="Two taboola postings. 8035268's `content` is a pasted "
                 "ChatGPT web UI; 8087797 is an ordinary posting from the same "
                 "board as a control for description normalization."):
    ...
    descriptions = {label: ats.greenhouse_description(body.get("content")) or ""
                    for label, body in bodies.items()}
    if not all(descriptions.values()):
        raise RuntimeError("a recorded posting no longer has a description")
    return "2 greenhouse posting descriptions"
...
def record_ats_lever():
    ...
    with cassettes.recording(
            ...
            note="ingest/ats.py fetch_lever('finix'); the only lever token in "
                 ...
                 "differs from older recordings. Finix returns a "
...
def record_ats_ashby():
    ...
    with cassettes.recording(
            ...
            note="ingest/ats.py fetch_ashby('runway'); smallest ashby board in "
                 ...
                 "The URL carries ?includeCompensation=true. "
...
def record_nyc_open_data():
    ...
    with cassettes.recording(
            ...
            note=f"ingest/nyc-open-data.py fetch_count/fetch_all/fetch_count "
                 ...
                 f"Both null post_until edge cases -- every null post_until "
...
ATS_VALIDATION_PROBES = (
    ...
    ("icims", "no-such-tenant-xyzzy", {},
     "iCIMS publishes no JSON feed, so "
...
#: an `Experienced-Hires` site with a different response shape; nyp 367 and
#: nordstrom 862 are both several times the bytes for no extra shape).
#: The source names
#: this tenant, this size and this request count.
#:
#: All three coordinates, not just the token:
...
def record_workday_cxs():
    ...
    WALK: it is a single page, and every multi-page failure in
     is about what the second page does.
    ...
    (The board was 88 over five pages at initial validation. It moved before
