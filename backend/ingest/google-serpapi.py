...
INSTALL: lives in ~/apps/jobs/backend alongside config/google-queries.json
...
CONFIG:
    ...
    SERPAPI_API_KEY            -- required, no fallback default (unlike
                                  ...
                                  ~/apps/jobs/backend/.env, never hardcoded here)
...
def main():
    ...
    for q, last_run_str in picked:
        ...
        try:
            ...
            # decides. mark_success still runs below -- the SerpApi credit is
            # spent either way and re-running the query would spend it twice.
            result = e.result
            query_errors.append(f"{q['slug']}: {e}")
        else:
            queries_run += 1
    ...
    if totals.new or totals.updated or closed_count or query_errors or totals.errors:
        print(f"google-jobs-serpapi-ingest: {totals.new} new, {totals.updated} updated, "
    ...
    try:
        ...
        print(f"google-jobs-serpapi ingest FAILED: {e}")
        sys.exit(1)
