...
USAGE:
    SERPAPI_API_KEY=... python3 tools/verify-date-filter.py
    python3 tools/verify-date-filter.py --query "LLM engineer" --location "New York, NY"
    python3 tools/verify-date-filter.py --dry-run    # print the plan, spend nothing
"""
...
def main():
    ...
    if not SERPAPI_API_KEY:
        print("FAILED: SERPAPI_API_KEY not set (it lives in ~/apps/jobs/backend/.env).",
              file=sys.stderr)
        return 1
