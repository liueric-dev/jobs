...
confirmed wrong and deliberately unfixed**, one table row outside `T-50`'s scope:
`frontend/README.md:64` cites `auth.py:450` for `GET /v1/me`; `:450` is a `conn.commit()`, route
`:465`. Two never-right in four is why this cannot be swept by offset.

**Done when:** all 104 name the line carrying their claim, read at the target, never by offset; both
checkers pass; `audit-citations.py` prints `0 new`; **nothing added to `config/citation-baseline.json`**.
