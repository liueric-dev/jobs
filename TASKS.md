...
**This file owns the prefix `T-`.** One allocator. **The next free number is `T-50`.** Numbers are
...
live pipeline behaviour rather than adding to it. `T-38` and `T-50` are the exceptions to the
dependency order and belong at the front: neither is a cut of `0007`, and each is a gap a closed row
opened or found and could not itself close (`T-26` and `T-49` respectively). **`T-50` is now the
front** — 75 `jobs.py` citations in `frontend/`, swept and counted but not one of them read yet.
...
Ten open rows, eight of them cutting `0007` into buildable steps and `T-38` and `T-50`
...
### T-50 — 75 `jobs.py` citations in `frontend/` nobody has read at the target

Filed by `T-49`, 2026-08-08, which corrected 22 numbers at 19 sites and drew its scope at the two
`MANIFEST.json` fixture objects and the one `frontend/README.md` bullet it was pointed into.
**`frontend/` holds 101 `jobs.py` citations; `T-48` and `T-49` between them have read 26.** The
other 75 span `frontend/README.md` (14), `fixtures/shipped/MANIFEST.json` (19),
`fixtures/contract/MANIFEST.json` (5), `check_client.mjs` (1) and ten modules under `js/`. Every one
resolves, so `audit-citations.py` is green across all — `T-40`'s blindspot for the sixth row running.

**They are a mix, and that is the whole difficulty.** At least eleven name a line that is **blank**
today and so cannot be carrying anything: `frontend/README.md:61` (`jobs.py:336`) and `:644`,
`MANIFEST.json:42` (`:346-347`), `:75` (`:313-317`) and `:88` (`:82-90`), `crawl.mjs:40` (`:346`),
`detail.mjs:212`/`:234` and `events.mjs:87` (`:82-90`), `saved.mjs:7`, `tracks.mjs:149`. Others land
mid-docstring on unrelated prose. **But some still hold** — `crawl.mjs:25` and `saved.mjs:14` on
`jobs.py:40` (`MAX_LIMIT = 100`), `detail.mjs:254` and `MANIFEST.json:83` on `:52-53`
(`CLIENT_EVENT_NAMES`) — so this cannot be swept with one offset, and a row that tries will re-break
the ones that are right: `jobs.py` did not move by a constant, and `T-49` measured shifts from +17
to +58 within the one file. **Do not assume drift, either**: six of the numbers in `T-49`'s own two
blocks were never right at any commit — `T-43`'s class, unfixable by mapping the old line forward.

**Done when:** all 75 name the line carrying their claim, read at the target and never by offset;
both frontend checkers pass; `audit-citations.py` still prints `0 new`; **nothing added to
`config/citation-baseline.json`**, since all 75 resolve today.
