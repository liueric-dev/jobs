...
**This file owns the prefix `T-`.** One allocator. **The next free number is `T-48`.** Numbers are
...
pipeline behaviour rather than adding to it. `T-38`, `T-46` and `T-47` are the exceptions to the
dependency order: none is a cut of `0007`, and they belong at the front rather than at the end.
`T-38` and `T-46` are each a gap a closed row opened or found and could not itself close (`T-26`
and `T-40` respectively); `T-47` is the one nobody found, because it has been flagged at the end of
...
`T-46` — the same defect in `api/app.py`, at twelve sites rather than one — and `T-47`, which is the
budget overrun both task files have carried unfiled for a week. **`T-46` is now the front of that
queue** — the last open row that is neither a cut of `0007` nor waiting on someone.
...
Eleven open rows, eight of them cutting `0007` into buildable steps and `T-38`, `T-46` and
`T-47` consequences of rows that closed. `0007` supersedes
...
### T-46 — `T-42` corrected six `api/app.py` citations; the next commit moved them all again

Found while closing `T-40`, in the file `T-40` is not about — the same defect at a scale `T-40`'s
single instance does not suggest. `T-42` read six citations into `api/app.py` at their targets and
corrected each; `T-39` closed **after** it, in `cd214ae`, which is `+7 / -76` against that file.
`api/app.py:580-587` says outright what that does: ~45 citations of the form `backend/api/app.py:NNN`
live across the tag's tree and three task files, inserting anything above `submit()` invalidates all
of them at once, and task 24 already re-cited about thirty after an eight-line shift. That comment is
why two constants sit at the bottom of the file. **`T-39` deleted 76 lines above `submit()` anyway.**

**Twelve citing sites naming sixteen numbers were read at their targets and are wrong, and that is a
floor rather than a survey:** `webapp/label.py:25` (`:209`, now a `MintRequest` field —
`authenticate()` is at `:126`) and `:84` (`:460`); `google-serpapi-worker.py:530` (`:431`), `:580`
(`:285`; `/v1/health` is at `:216`), `:619` (`:361`, plus `query_claims.py:196`, which is
`REQUIRED_COLUMNS` and not a claim lock) and `:624` (`:549`, `:551`); `api/tests/test_worker_check.py:195`
(`:431`); `DEV_TASKS.md:497` (`:580`, `:582` — which now land on the comment quoted above, about
citations going stale) and `:580` (`:428`); `T-35`'s own row (`:380`, `:383`); `T-30`'s closed row
(`:549`, `:551`, `:269`). Correct, and to be left alone: `evals/labels.py:374` and
`docs/STATE-OF-THE-SYSTEM.md:223`, both `:99`. The checker prints `0 new` across all of it.

**Two are load-bearing for an open row**: `T-35` is unbuilt and its own text carries `:380`/`:383`,
so a session standing on it reads `submit()`'s signature where it was promised `claim`'s two halves.

**Done when:** each citation names the lines carrying its claim, **read at the target, never
offset** — `T-42` found three of its six had moved by amounts differing from the other three, and
`T-40` found two that were never right, so arithmetic reproduces neither. The checker still prints
`0 new`, and **nothing is added to `config/citation-baseline.json`**: every one resolves, so the
baseline would record the opposite of what is wrong. **Do not build a content checker** — `T-40`
declined to and was right; pin a claim worth pinning in the file that makes it, as `T-40` did.
`OQ-33` asks whether this row should have to exist at all, and does not block it.

---

### T-47 — `DEV_TASKS.md` is 673 lines against a 450 budget and has never been a row
