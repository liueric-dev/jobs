...
**This file owns the prefix `T-`.** One allocator. **The next free number is `T-49`.** Numbers are
...
### T-48 — the same defect in `webapp/app.py`, at four sites

Found while closing `T-46`, by sweeping for `app.py:NNN` rather than `api/app.py:NNN`. **That is
also why `T-46`'s own list was a floor**: the narrower pattern misses every citation written without
the directory, and `frontend/` and `api/tests/` both write it that way.

**Four sites, read at the target.** `frontend/js/api.mjs:14` and `frontend/README.md:315` cite
`:93` for the registration of the `ContractError` envelope, which is `backend/webapp/app.py:101`;
`frontend/README.md:103` and `frontend/verify_fixtures.py:443` cite `:99` for the mounted onboarding
router, which is `:106`. Both wrong numbers land in the comment block *above* the statement they
name, so both resolve and `tools/audit-citations.py` is green across them — `T-40`'s blindspot for
the fourth row running. **Correct, and to be left alone:** `docs/STATE-OF-THE-SYSTEM.md:381` (`:84`,
the `allow_methods` literal) and `frontend/serve.py:18` (`:78-84`, the CORS allowlist).

**Two of the four are in `frontend/`, which ships without a build step** — see
`.claude/rules/frontend.md`. Nothing here is executable, but `frontend/verify_fixtures.py` is one of
the two checkers CI runs, so edit and re-run it rather than assuming a comment is inert.

**Not in scope, and already filed:** the `../STATE-OF-THE-SYSTEM.md:443-448` citations in
`docs/adr/` are `T-43`, which already names `:459-464` as the real target and is gated on the freeze
question. `T-46` deliberately left them rather than half-renumber a set `T-43` owns.
