...
blocked behind it for exactly that reason. **`T-40` is now the front of that queue** — the last
open row that is neither a cut of `0007` nor waiting on someone.
...
Ten open rows, eight of them cutting `0007` into buildable steps and `T-38` and `T-40`
...
**Baseline: all three suites are green — `1462` / `397` / `274`, all `OK`, re-measured 2026-08-08
after `T-45`.** `T-39` added 6 to the pipeline suite (`tests/test_provision_covers_api_schema.py`)
...
`api/tests/test_poll_interval.py`, `273` after `T-39`'s one and `274` after `T-45`'s — **this
